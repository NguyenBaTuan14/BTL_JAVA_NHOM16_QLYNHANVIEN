       ...
       if (check()) {
           if (index == -1) {
               ...
               if (JOptionPane.showConfirmDialog(this,
                       "Bạn có chắc chắn muốn sửa thông tin không?", "Cập nhật", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
...
//       btnGioiTinh.clearSelection();
   ...
   private boolean check() {
       ...
       for (NhanVien nv : NhanVienService.nhanViens) {
           if (txtMaNV.getText().equalsIgnoreCase(nv.getMa()) && !txtMaNV.getText().equalsIgnoreCase(tblNhanViens.getValueAt(tblNhanViens.getSelectedRow(), 0).toString())) {
